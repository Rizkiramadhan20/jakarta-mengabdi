...
export default function Kakasakutransaction() {
    ...
    const filteredTransactions = transactions.filter(trx => {
        // Hanya tampilkan transaksi dengan status settlement
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [search, kakaSakuFilter, dateFilter]);
    ...
    return (
        <section>
            ...
            {(search || kakaSakuFilter !== 'all' || dateFilter) && (
                <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div className="text-sm text-green-800">
                        Menampilkan {paginatedTransactions.length} dari {filteredTransactions.length} transaksi selesai
            ...
            <Card className="mt-6">
                <CardHeader>
                    <CardTitle>
                        ...
                        {filteredTransactions.length !== transactions.filter(t => t.status === 'settlement').length && (
                            <span className="text-sm font-normal text-gray-500 ml-2">
                                ({filteredTransactions.length} dari {transactions.filter(t => t.status === 'settlement').length})
                            </span>
                        )}
                        {filteredTransactions.length === transactions.filter(t => t.status === 'settlement').length && (
                            <span className="text-sm font-normal text-gray-500 ml-2">
                                ({transactions.filter(t => t.status === 'settlement').length})
                            </span>
                        )}
                    </CardTitle>
                </CardHeader>
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col justify-center items-center py-8">
                            <p className="text-gray-400 text-lg mb-4">
                                {search || kakaSakuFilter !== 'all' || dateFilter
                                    ? 'Tidak ada transaksi selesai yang sesuai dengan filter yang dipilih.'
                        ...
                        <div className="flex flex-col justify-center items-center py-8">
                            <p className="text-gray-400 text-lg mb-4">
                                Tidak ada transaksi selesai di halaman {currentPage}.
