...
export default function VolunteerLayout({ volunteerData }: VolunteerLayoutProps) {
    ...
    // State untuk opsi harga terpilih
    const [selectedPaymentIdx, setSelectedPaymentIdx] = useState(0);
    const paymentOptions = Array.isArray(volunteerData.payment_options) ? volunteerData.payment_options : [];
    const selectedPayment = paymentOptions[selectedPaymentIdx];
                    ...
                    <div className="flex-1 flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-1 gap-6">
                            ...
                            <div className="bg-[#F8F5F2] rounded-xl shadow p-6 border col-span-1">
                                ...
                                <p className="text-gray-700 text-sm leading-relaxed">{volunteerData.description}</p>
                            </div>
                            {/* Detail Aktivitas */}
                            <div className="bg-[#F8F5F2] rounded-xl shadow p-6 border col-span-1">
                                <div className="flex items-center gap-2 text-base font-bold mb-2">
                            ...
                            <div className="bg-[#F8F5F2] rounded-xl shadow p-6 border col-span-1">
                                ...
                                <div className='p-4 bg-white rounded-lg flex flex-col gap-2'>
                                    <div className='flex items-center gap-2'>
                                        ...
                                        <span className='text-sm text-gray-700'>{volunteerData.devisi[0]}</span>
                                    </div>
  ...
  "
                    ...
                    <div className="space-y-6 lg:sticky lg:top-28 lg:self-start w-full lg:w-[400px] flex-shrink-0">
                        <div className="bg-white rounded-2xl shadow-lg p-6 border border-orange-200">
                            {/* Badge Kategori */}
                            <div className="mb-3">
                                <span className={`inline-block ${color.bg} ${color.text} text-xs font-bold px-4 py-1 rounded-full`}>
                                    {categoryName}
                                </span>
                            </div>

                            {/* Judul */}
                            <h2 className="text-xl font-bold text-gray-900 leading-tight mb-3">{volunteerData.title}</h2>

                            {/* Harga */}
                            {paymentOptions.length > 0 ? (
                                <>
                                    <div className="flex items-center gap-4 mb-4">
                                        <Select value={selectedPaymentIdx.toString()} onValueChange={val => setSelectedPaymentIdx(Number(val))}>
                                            <SelectTrigger className="min-w-[120px]">
                                                <SelectValue>
                                                    {paymentOptions[selectedPaymentIdx].type.charAt(0).toUpperCase() + paymentOptions[selectedPaymentIdx].type.slice(1)}
                                                </SelectValue>
                                            </SelectTrigger>
                                            <SelectContent>
                                        ...
                                        <div className="text-2xl font-extrabold text-green-700">
                                            {selectedPayment.type === 'gratis' ? 'Gratis' : `Rp ${formatIDR(Number(selectedPayment.price))}`}
                                        </div>
                                    </div>
                                </>
                            ) : (
                                <div className="text-2xl font-extrabold text-green-700 mb-4">-</div>
                            )}
                            {/* Box Info - Redesigned */}
                            <div className="relative border rounded-xl shadow p-4 mb-5 bg-white pb-12">
                                ...
                                <div className="flex items-start gap-3 mb-4">
                                    ...
                                    <div>
                                        <div className="text-xs text-gray-500 font-medium mb-0.5">Jadwal event</div>
                                        <div className="text-sm font-semibold text-gray-600">
                                            {volunteerData.time ? formatDateIndo(volunteerData.time) : '-'}
                                        </div>
                                        <div className="text-xs text-gray-600">
                                            Pukul: {volunteerData.time ? formatTimeIndo(volunteerData.time) : '-'}
                                        </div>
                                    </div>
                                </div>
                                {/* Lokasi */}
                                <div className="flex items-start gap-3 mb-4">
                                    ...
                                    <div>
                                        <div className="text-xs text-gray-500 font-medium mb-0.5">Lokasi</div>
                                        <div className="text-sm font-semibold text-gray-600">{volunteerData.location}</div>
                                    </div>
                                </div>
                                ...
                                <div className="absolute left-0 right-0 bottom-0 rounded-t-xl bg-[#FFF3E0] border-t border-orange-200 px-3 py-3 flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-gray-600 font-medium">
                                        Batas registrasi: {volunteerData.last_time ? formatDateIndo(volunteerData.last_time) : '-'}
                                    </span>
                                </div>
                            </div>
